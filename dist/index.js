...
async function main() {
    ...
    // eslint-disable-next-line no-console
    console.log("hot files (files with most changes)", Object.keys(commitsPerFile)
        .map((x) => {
        return [x, commitsPerFile[x]];
    })
        .sort((a, b) => Number(b[1]) - Number(a[1])));
    // eslint-disable-next-line no-console
    console.log("knowledge gaps (files with least number of contributors)", Object.keys(contributorsPerFile)
        .map((x) => {
